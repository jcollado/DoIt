...
ko.bindingHandlers.draggable = {
  init: function(element) {
    $(element).draggable({
      'helper': function() {
        var taskSeries = ko.dataFor(this);
        return $('<p class="box ui-dragging">' + taskSeries.name() + '</p>');
      },
      'revert': 'invalid',
      'opacity': 0.8,
...
ko.bindingHandlers.droppable = {
  init: function(element) {
    $(element).droppable({
      'accept': function(a, b) {
        // Tasks can be moved to any list, but the one that is currently selected
        var list = ko.dataFor(this);
        return !list.selected();
      },
      'drop': function(event, ui) {
        var fromList = ko.dataFor($('.list.selected')[0]);
        var toList = ko.dataFor(this);
        var taskSeries = ko.dataFor(ui.draggable.get(0));
        console.log(
          'Moving ' + taskSeries.name() +
          ' from ' + fromList.name() +
          ' to ' + toList.name() + '...');
        taskSeries.move(fromList, toList);
      },
...
function ListModel(app, data) {
  var self = this;
  var _self = _(this);
  var booleanProperties = _(['archived', 'deleted', 'locked']);
  ...
  this.displayActions = ko.computed(function() {
    return !self.locked && self.hover() && !self.waitingForResponse();
  });


  /**
   * List name is being edited
   */
  ...
  this.name.subscribe(function(name) {
    ...
    console.log('List name updated:', name);

    app.rtm.timelines.create(authToken, function(data) {
      var timeline = data.rsp.timeline;

      app.rtm.lists.setName(
        authToken,
        timeline,
        self.id,
        name,
...
function TaskSeriesModel(app, data) {
   ...
   * @param {ListModel} fromList - List where the task is now
   * @param {ListModel} toList - List where the task is to be moved to
   */
  this.move = function(fromList, toList) {
    var authToken = app.getAuthToken();
    self.waitingForResponse(true);
    fromList.waitingForResponse(true);
    toList.waitingForResponse(true);

    app.rtm.timelines.create(authToken, function(data) {
      var timeline = data.rsp.timeline;

      app.rtm.tasks.moveTo(
        authToken,
        timeline,
        fromList.id,
        toList.id,
        ...
        function() {
          self.display(false);
          fromList.waitingForResponse(false);
          toList.waitingForResponse(false);
        }
      );
    });
  };
  ...
  this.name.subscribe(function(name) {
    ...
    console.log('Task name updated:', name);

    app.rtm.timelines.create(authToken, function(data) {
      var timeline = data.rsp.timeline;

      app.rtm.tasks.setName(
        authToken,
        timeline,
        app.selectedList().id,
        self.id,
  ...
  // Map due date to Date object to sort it easily
  this.task.dueDate = this.task.due ? new Date(this.task.due) : null;

  // Set default text for due date/time
  this.task.due = this.task.due || 'No due date/time set';

  // Map priority to integer, so that it can be sorted
  this.task.priorityNumber = priorityNumberMap[this.task.priority];

  // Map priority to readable string
  this.task.priority = priorityMap[this.task.priority];

  // Map tags properly to an array object
  tags = this.tags.tag;
  if (_.isArray(tags)) {
    this.tags = tags;
...
function ApplicationModel(rtm) {
  ...
  this.sortedLists = ko.computed(function() {
    return _.sortBy(self.lists(), function(list) {
   ...
   */
  this.taskSortBy = ko.observable('name');

  this.sortedTaskSeries = ko.computed(function() {
    // Always sort by name and then by some other criteria
    var sortedTaskSeries = _.sortBy(self.taskSeries(), function(taskSeries) {
          return taskSeries.name().toLowerCase();
        });

    switch (self.taskSortBy()) {
      case 'priority':
        sortedTaskSeries = _.sortBy(sortedTaskSeries, function(taskSeries) {
          return taskSeries.task.priorityNumber;
        });
        break;
      case 'due':
        taskSeriesWithDueDate = _.filter(sortedTaskSeries, function(taskSeries) {
          return taskSeries.task.dueDate;
        });
        console.log('Task series with due date', taskSeriesWithDueDate);

        taskSeriesWithoutDueDate = _.filter(sortedTaskSeries, function(taskSeries) {
          return !taskSeries.task.dueDate;
        });
        console.log('Task series without due date', taskSeriesWithoutDueDate);

        sortedTaskSeries = _.sortBy(taskSeriesWithDueDate, function(taskSeries) {
          return taskSeries.task.dueDate.getTime();
        }).concat(taskSeriesWithoutDueDate);
    }

    return sortedTaskSeries;
  });
   ...
   */
  this.invalidListName = ko.computed(function() {
    return !(/\S+/).test(self.newListName());
  });
   ...
   */
  this.invalidTaskName = ko.computed(function() {
    return !(/\S+/).test(self.newTaskName());
  });
   ...
   * Select list
   *
   * Update location URL so that sammy takes over an displays selected task
   */
  ...
  /**
   * Load list to display its tasks
   *
   * @param {List} list - List to be loaded
   */
  this.loadList = function loadList(list) {
    var authToken = self.getAuthToken();
  ...
  Sammy(function() {
    ...
    /*
     * Load list by name in the hash URL
     */
    this.get('#:listName', function() {
      var listName = this.params.listName;
      var list = _.find(self.lists(), function(list) {
        return listName == list.name();
      });
      if (_.isUndefined(list)) {
        console.log('Route for list ' + listName + ' not found');
        this.app.runRoute('get', '');
      } else {
        console.log('Route for list: ' + listName);
        self.loadList(list);
      }
    });

    /*
     * Default view is no list selected
     */
    this.get('', function() {
      console.log('Default route');
      var selectedList = self.selectedList();
      self.taskSeries.removeAll();
      if (!_.isUndefined(selectedList)) {
        selectedList.selected(false);
      }
      location.hash = '';
    });
    ...
    /*
     * Redirect to default view on error
     */
