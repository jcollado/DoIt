 ...
 * @returns {List} The new List object
 */
function List() {
}
 ...
 *
 * Properties are returned by RememberTheMilk as strings. This method makes
 * things easier by mapping them to the correct boolean value
 */
List.prototype.initialize = function initialize() {
  var self = this;

  _.each(['archived', 'deleted'], function(propertyName) {
    self[propertyName] = Boolean(parseInt(self[propertyName], 10));
  });
...
function ApplicationModel(rtm) {
  ...
  /**
   * Retun whether the give list is the selected list
   *
   * @param {List} list - A list
   */
  this.isSelectedList = function isSelectedList(list) {
    var selectedList = self.selectedList();
    return selectedList && selectedList.id == list.id;
  };
  ...
  function authorized() {
    ...
    rtm.lists.getList(getAuthToken(), function(data) {
      ...
      var lists = _.map(data.rsp.lists.list, function(list) {
        // .initialize is used to parse boolean values correctly
        return _.extend(new List(), list).initialize();
      });
  ...
  this.selectList = function selectList(list) {
    ...
    if (self.state() == 'ready') {
      ...
      rtm.tasks.getList(authToken, list.id, function(data) {
        ...
        self.taskSeries.removeAll();
        _.each(taskSeries, function(ts) {
          self.taskSeries.push(ts);
        });
